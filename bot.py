...
BLACKLIST_FILE = "blacklist.txt" # ملف للإيميلات المعطوبة
...
async def send_email_with_cv(target_email):
    ...
    try:
        ...
        msg['From'] = EMAIL_USER
        msg['To'] = clean_email
        msg['Subject'] = f"Request for Job Opportunity - HS Graduate - {random.randint(100, 999)}"
        ...
        # إذا فشل، نضيفه للقائمة السوداء فوراً عشان ما يوقفنا بكره
        with open(BLACKLIST_FILE, "a") as f:
            f.write(target_email + "\n")
        print(f"⚠️ تم حظر الإيميل المعطوب: {target_email}")
        return False

async def get_fresh_emails(page):
    # كلمات بحث "نظيفة" وجديدة كلياً لعام 2026
    queries = [
        'site:tanqeeb.com "السعودية" "ثانوي" "gmail"',
        'site:sa.jooble.org "الدمام" "ثانوي" "إيميل"',
        'site:sa.indeez.com "وظائف" "ثانوي" "اتصل بنا"',
        '"إرسال السيرة الذاتية" ثانوي الخبر 2026'
    ]
    found_emails = set()
    for query in queries:
        try:
            ...
            for e in emails:
                if not any(x in e.lower() for x in ['google', 'facebook', 'instagram', 'sentry', 'w3.org']):
...
async def run_bot():
    async with async_playwright() as p:
        ...
        print("🚀 تشغيل وضع 'الاكتساح' - تنظيف وحظر الإيميلات المعطوبة")
        
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page()
        discovered_emails = await get_fresh_emails(page)
        
        # تحميل القائمة السوداء والقديمة
        ignore_list = set()
        ...
        print(f"🎯 المستهدف اليوم بعد الفلترة: {len(to_apply)} جهة توظيف.")
